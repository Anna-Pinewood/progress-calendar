...
*   **Проверка:**
    *   После запуска API (и выполнения миграций, если это автоматизировано ORM) в базе данных PostgreSQL существуют таблицы `spheres` и `achievements` с корректными колонками, типами данных и связями (foreign key).
    *   API успешно подключается к БД при старте (логировать успешное подключение).
